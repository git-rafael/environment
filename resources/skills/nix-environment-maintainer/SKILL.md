...
- symlinks under `resources/skills/`
...
Output approach: add or link the skill under `resources/skills/`, explain that `sources/shell.nix` already exposes that tree to the supported agent directories, then suggest applying the Home Manager environment after a local commit.
